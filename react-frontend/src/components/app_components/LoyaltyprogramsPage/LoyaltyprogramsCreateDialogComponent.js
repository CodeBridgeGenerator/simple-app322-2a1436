...
const LoyaltyprogramsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            partId: _entity?.partId,partName: _entity?.partName,description: _entity?.description,quantityInStock: _entity?.quantityInStock,price: _entity?.price,supplierId: _entity?.supplierId,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Loyaltyprograms created successfully" });

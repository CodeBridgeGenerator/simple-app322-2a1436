...
import suppliersFakerFactory from "./suppliersFakerFactory";
...
const SuppliersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = suppliersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("suppliers").create(elem));
...
export default connect(mapState, mapDispatch)(SuppliersFakerDialogComponent);

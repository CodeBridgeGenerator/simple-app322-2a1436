...
import servicesFakerFactory from "./servicesFakerFactory";
...
const ServicesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = servicesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("services").create(elem));
...
export default connect(mapState, mapDispatch)(ServicesFakerDialogComponent);

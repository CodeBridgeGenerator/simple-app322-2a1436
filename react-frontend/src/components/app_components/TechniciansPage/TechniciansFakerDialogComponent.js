...
import techniciansFakerFactory from "./techniciansFakerFactory";
...
const TechniciansFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = techniciansFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("technicians").create(elem));
...
export default connect(mapState, mapDispatch)(TechniciansFakerDialogComponent);

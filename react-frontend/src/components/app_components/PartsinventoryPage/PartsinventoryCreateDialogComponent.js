...
const PartsinventoryCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Partsinventory created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Partsinventory" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Partsinventory" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
